...
import { Currency, currencyEquals, ETHER, Token } from '@solarswap/sdk'
...
export function useCurrency(currencyId: string | undefined): Currency | Token | null | undefined {
	const isASA = currencyId?.toUpperCase() === 'ASA' || currencyId?.toLowerCase() === GELATO_NATIVE
	const token = useToken(isASA ? undefined : currencyId)

	return isASA ? ETHER : token
}
