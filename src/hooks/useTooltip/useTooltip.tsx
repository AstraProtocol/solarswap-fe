...
import styleClass from './styles.module.scss'
...
const useTooltip = (content: React.ReactNode, options: TooltipOptions): TooltipRefs => {
	...
	const tooltip = (
		<div
			className={clsx(styleClass.tooltip, 'same-bg-color-100')}
			ref={setTooltipElement}
			style={styles.popper}
			{...attributes.popper}
		>
			<div className="contrast-color-100">{content}</div>
			<div
				className={clsx('same-bg-color-100 same-color-100', styleClass.arrow)}
				ref={setArrowElement}
				style={styles.arrow}
			/>
		</div>
