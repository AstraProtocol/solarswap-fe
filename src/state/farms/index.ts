...
>(
	...
	},
	{
		condition: (arg, { getState }) => {
			const { farms } = getState()
			// console.log('fetchFarmUserDataAsync farms :>> ', farms)
			if (farms.loadingKeys[stringify({ type: fetchFarmUserDataAsync.typePrefix, arg })]) {
				console.log('farms user action is fetching, skipping here')
				return false
			}
			// console.log('farms user action is fetching, skipping here after')
			return true
		},
	},
)
...
export const farmsSlice = createSlice({
	...
	extraReducers: builder => {
		...
		builder.addMatcher(
			isAnyOf(fetchFarmUserDataAsync.pending, fetchFarmsPublicDataAsync.pending),
			(state, action) => {
