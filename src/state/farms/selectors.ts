...
const selectAsaFarm = (state: State) => state.farms.data.find(f => f.pid === 1)
...
export const priceAstraFromPidSelector = createSelector([selectCakeFarm], astraBnbFarm => {
	const deserializedCakeBnbFarm = deserializeFarm(astraBnbFarm)
	const asaPriceBusdAsString = deserializedCakeBnbFarm.tokenPriceBusd
	return new BigNumber(asaPriceBusdAsString)
})

export const priceAsaFromPidSelector = createSelector([selectAsaFarm], asaUsdtFarm => {
	const deserializedAsaUsdtFarm = deserializeFarm(asaUsdtFarm)
	const asaPriceUsdtAsString = deserializedAsaUsdtFarm.quoteTokenPriceBusd
	return new BigNumber(asaPriceUsdtAsString)
})
