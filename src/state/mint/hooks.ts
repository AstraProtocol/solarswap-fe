import { Currency, CurrencyAmount, ETHER, JSBI, Pair, Percent, Price, TokenAmount } from '@solarswap/sdk'
...
import useZapInAmount from 'hooks/useZapInAmount'
...
export function useDerivedSingleMintInfo(
	...
	// pair
	const [pairState, pair] = usePair(currencyIn, currencyOut)

	const totalSupply = useTotalSupply(pair?.liquidityToken)

	const [tokenIn, tokenOut] = [wrappedCurrency(currencyIn, chainId), wrappedCurrency(currencyOut, chainId)]

	const pairAddress = pair?.liquidityToken?.address
	// amounts
	const independentAmount: CurrencyAmount | undefined = tryParseAmount(typedValue, currencyIn)
	const dependentAmount: CurrencyAmount | undefined = undefined

	const [tokenInAmount, tokenOutAmount] = useZapInAmount(
		tokenIn?.address,
		tokenOut?.address,
		pairAddress,
	...
	// balances
	const balances = useCurrencyBalances(account ?? undefined, [currencyIn, currencyOut])
	...
	const price = useMemo(() => {
		const wrappedCurrencyA = wrappedCurrency(currencyIn, chainId)
		return pair && wrappedCurrencyA ? pair.priceOf(wrappedCurrencyA) : undefined
	}, [chainId, currencyIn, pair])

	// liquidity minted
	const liquidityMinted = useMemo(() => {
		if (pair && totalSupply && tokenInAmount && tokenOutAmount) {
			try {
				const tokenAmounts = {
		...
		return undefined
	}, [pair, totalSupply, tokenInAmount, tokenOutAmount, independentField, tokenIn, dependentField, tokenOut])
	...
	if (!parsedAmounts[independentField]) {
		error = error ?? t('Enter an amount')
	}

	const { [independentField]: currencyInAmount } = parsedAmounts

	if (currencyInAmount && currencyBalances?.[independentField]?.lessThan(parsedAmounts[independentField])) {
		error = t('Insufficient %symbol% balance', { symbol: currencies[independentField]?.symbol })
	}

	if (!liquidityMinted && typedValue) {
		error = t('Insufficient liquidity for this trade.')
	}
