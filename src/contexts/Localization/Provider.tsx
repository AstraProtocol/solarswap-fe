	...
	useEffect(() => {
		const fetchInitialLocales = async () => {
			...
			if (codeFromStorage !== VI.locale) {
				...
				if (currentLocale) {
					const currentLocaleWithIncludesVariables =
						getTranslationsWithIncludesVariable(currentLocale);
	...
	const setLanguage = useCallback(async (language: Language) => {
		if (!languageMap.has(language.locale)) {
			...
			if (locale) {
				...
				languageMap.set(language.locale, { ...viLocale, ...localeWithIncludesVariables });
			}
	...
	const translate: TranslateFunction = useCallback(
		(key, data) => {
			const translationSet =
				languageMap.get(currentLanguage.locale) ?? languageMap.get(VI.locale);
